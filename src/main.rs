...
fn load_manifest() -> Option<Manifest> {
    ...
    toml::from_str::<Manifest>(&content)
        .map_err(|e| tracing::error!(path = %path.display(), error = %e, "module.toml invalid"))
        .ok()
}
...
#[derive(Parser, Debug)]
#[command(name = "kubuno-forum", version, about = "Kubuno Forum module")]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    match settings.logging.format {
        kubuno_forum::config::LogFormat::Json   => subscriber.json().init(),
        kubuno_forum::config::LogFormat::Pretty => subscriber.init(),
    }

    tracing::info!("Kubuno Forum v{} starting…", env!("CARGO_PKG_VERSION"));

    // Security: forbid any process execution on the host.
    kubuno_seccomp::lock_down_process_execution("forum");
    ...
    if settings.database.run_migrations {
        sqlx::query("CREATE SCHEMA IF NOT EXISTS forum")
            .execute(&pool)
            .await
            .context("Creating the forum schema")?;

        let migration_opts = settings
            .database
            .connect_options()?
            .options([("search_path", "forum,public")]);
    ...
    // Heartbeat every 30s
    {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let url    = format!("{}/internal/modules/forum/heartbeat", settings2.core.url);
                ...
                match http2.post(&url).header("X-Internal-Secret", secret.as_str()).send().await {
                    ...
                    Ok(r)  => tracing::warn!(status = %r.status(), "Heartbeat unexpected response"),
    ...
    let listener = tokio::net::TcpListener::bind(&addr)
        .await
        .with_context(|| format!("Bind on {addr}"))?;

    tracing::info!("Kubuno Forum started on http://{addr}");
...
async fn register_with_core(http: &Client, settings: &Settings) {
    ...
    let display_name  = manifest.as_ref().map(|m| m.module.display_name.as_str()).unwrap_or("Forum").to_string();
    ...
    let sidebar_items: Vec<Value> = manifest.as_ref()
        ...
        .unwrap_or_else(|| vec![
            json!({ "id": "forum", "label": "Forum", "icon": "MessagesSquare", "path": "/forum", "position": 24 }),
        ]);
    ...
    for attempt in 1u32.. {
            ...
            .await
        {
            Ok(resp) if resp.status().is_success() => {
                tracing::info!("Module forum registered with the core");
                return;
            }
