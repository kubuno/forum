#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# build_deb.sh — builds the Debian package for the Kubuno Forum module.
#
# Produces the SAME install layout as the monorepo, so the core discovers the
# module identically:
#   /usr/lib/kubuno/modules/forum/{kubuno-forum, module.toml, frontend/}
#   /usr/share/kubuno/modules/forum/migrations/*.sql
#   /etc/kubuno/modules/forum/config.toml.example
#
# Usage:
#   bash build_deb.sh            # build the .deb into dist/
#   bash build_deb.sh --install  # build then `apt install` the .deb
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
cd "$(dirname "$0")"
export SQLX_OFFLINE=true   # use the .sqlx cache (no DB at build time)
...
Description: Kubuno Forum — discussion boards (categories, forums, topics, posts) (build ${BUILD_NUM})
EOF
